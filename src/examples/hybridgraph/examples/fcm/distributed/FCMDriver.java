...
/**
 * FCMDriver.java
 * A driven program is used to submit the Fuzzy C-Means computation job.
 * 
 * @author 
 * @version 0.1
 */
public class FCMDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length != 14) {
			StringBuffer sb = 
				new StringBuffer("the Fuzzy C-Means job must be given arguments(14):"); 
		...
		BSPJob bsp = new BSPJob(conf, FCMDriver.class);
		bsp.setPriority(Constants.PRIORITY.NORMAL);
		bsp.setJobName("Fuzzy C-Means");
		
		bsp.setBspClass(FCMBSP.class);
		bsp.setUserToolClass(FCMUserTool.class);
