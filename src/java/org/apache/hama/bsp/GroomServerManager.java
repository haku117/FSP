...
interface GroomServerManager {
  ...
  /**
   * NEU change in version-0.2.5 new function: update the GroomServerStatus Cache(now it is used in SimpleTaskScheduler and BSPMaster)
   * @param old  the original GroomServerStatus, it will be replaced by the new GroomServerStatus.
   * @param new  the new GroomServerStatus.
   */
  void updateGroomServersKey(GroomServerStatus old, GroomServerStatus newKey);
  ...
  void setWaitTasks(int waitTasks);
}
