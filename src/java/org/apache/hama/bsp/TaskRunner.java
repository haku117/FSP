...
import org.apache.hama.myhama.util.TaskLog;
...
public class TaskRunner extends Thread {
  ...
  public void run() {
    try {
      ...
      long logSize = TaskLog.getTaskLogLength(((HamaConfiguration)conf.getConf()));
      vargs.add("-Dtermite.log.dir=" + 
          new File(System.getProperty("termite.log.dir")
          ).getAbsolutePath());
      vargs.add("-Dtermite.root.logger=INFO,TLA");
      vargs.add("-Dtermite.tasklog.taskid=" + task.getTaskID());
      vargs.add("-Dtermite.tasklog.totalLogFileSize=" + logSize);
      ...
      //bspPeerBarrier();
      // Run java
      LOG.info(task.getTaskAttemptId() + " starts with JVM=" + javaOpts);
  ...
  private void logStream(InputStream output) {
    try {
      ...
      String line;
      StringBuffer sb = new StringBuffer("[logStream] ");
      boolean flag = false;
      ...
      if (flag) {
    	  LOG.info(task.getTaskID() + " " + sb.toString());
      }
    } catch (IOException e) {
