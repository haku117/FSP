...
class JobInProgress {
	 ...
	 public class SemiAsynTimer extends Thread {
	      public void run() {
	          ...
	          while (true) {
	             ...
	             if (isSemiAsyn()) {
	            	 //LOG.info("initiate a synchronous barrier");
	...
	public void finishSuperStep(int parId, SuperStepReport ssr) {
		...
		if (finished == this.taskNum) {
			...
			this.jobInfo.recordIterationRuntime(time); 
			this.startTimeIte = System.currentTimeMillis();
			if (this.curIteNum == 1) {
				/*
				...
				this.interAdj = new IntervalAdjuster(time*1000.0);
				this.intervalOfSync = (int)(time*1000.0);
			...
			if (this.interAdj.isAdjust()) {
				this.intervalOfSync = 
					this.interAdj.computeNewInterval(
			...
			if (this.syncTime > 0) {
				this.interAdj.updateSyncTime(System.currentTimeMillis()-this.syncTime);
			}
	...
	public void saveResultOver(int parId, int saveRecordNum, int passes, double contributions) {
		this.jobInfo.collectInfo(parId, passes, contributions);
		int finished = this.reportCounter.incrementAndGet();
		if (finished == this.taskNum) {
			this.reportCounter.set(0);
