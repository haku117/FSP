...
public class BSPTask<V, W, M, I> extends Task {
	...
	//just for proposal2017
	private boolean isRevoked = false;
	private long elapsedTime = 0L;
	private String ckpTaskDir;
	private int ckpVersion = 0;
	...
	private void buildRouteTable(BSPPeerProtocol umbilical) throws Exception {
		...
		String _ckpJobDir = this.commServer.getJobInformation().getCheckPointDirAftBuildRouteTable();
		Path path = new Path(_ckpJobDir, "task-"+this.parId);
		FileSystem fs = path.getFileSystem(this.job.getConf());
		if (fs.mkdirs(path)) {
			LOG.info("\ncreate checkpoint dir:" + path.toString());
		} else {
			LOG.error("\nfail to create checkpoint dir:" + path.toString());
		}
		this.ckpTaskDir = path.toString();
	}
	...
	private void finishIteration() throws Exception {
		...
		LOG.info("===== superstep-" + iteNum + " is done =====\n");
	...
	private void saveResult() throws Exception {
		...
		this.reportServer.saveResultOver(jobId, parId, num, this.passes, this.contributions);
	...
	private void transient_archive() throws Exception {
		if ((this.parId<this.idleTaskNum) && (this.idleTime>0) && (this.iteNum>1) && (this.iteNum%13)==0 && (!this.isRevoked)) {
			if (this.ckpVersion == this.iteNum) {
				return;
			} else {
				this.ckpVersion = this.iteNum;
			}
			
			long start = System.currentTimeMillis();
			//archiving data;
			TextBSPFileOutputFormat outputFormat = new TextBSPFileOutputFormat();
			outputFormat.initialize(this.job.getConf());
			RecordWriter<Text, Text> output = 
				outputFormat.getRecordWriter(this.job, new Path(ckpTaskDir, "ckp-" + this.ckpVersion));
			for (int idx = 0; idx < this.points.length; idx++) {
				output.write(new Text(Integer.toString(this.points[idx].getVerId())), 
						new Text(this.points[idx].getVerValue().toString()));
			}
			output.close(this.job);
			
			this.isRevoked = true;
			long end = System.currentTimeMillis();
			LOG.info("the overhead of archiving mutable variables is: " + (end-start)/1000.0 + " seconds");
		}
	}
	
	private void transient_restart() {
		long interval = 50L;
		long remainingTime = 90*1000 - this.elapsedTime;
		if (this.parId<this.idleTaskNum && this.iteNum>1 && this.isRevoked && remainingTime>0) {
			while(remainingTime > 0) {
				try {
					Thread.sleep(interval); //1000ms => 1s
				} catch (InterruptedException e) {
					LOG.error("transient_restart()", e);
				}
				this.elapsedTime += interval;
				remainingTime -= interval;
				if (this.commServer.isSemiAsynBarrier()) {
					break;
				}
			}
			if (remainingTime <= 0) {
		...
		try {
			...
			while (this.conExe) {
				for (int idx = 0; idx < this.points.length; idx++) {
					...
					//simulate the slowest task.
					//idle();
					this.transient_archive();
					this.transient_restart();
					
					//for semi-asyn method
					if (this.syncModel==SyncModel.SemiAsyn 
							&& this.commServer.isSemiAsynBarrier()) {
						this.computeTime += (System.currentTimeMillis()-compTime);
						
						if (iteNum > 1) {
							this.contributions += this.taskAgg;
						}
