...
public class JobInformation implements Writable {
	...
	public JobInformation (BSPJob _job, int _taskNum) {
		...
		//just for validation
		this.passes = new int[_taskNum];
		this.contributions = new double[_taskNum];
	}
	
	public void collectInfo(int parId, int _passes, double _contributions) {
		this.passes[parId] = _passes;
		this.contributions[parId] = _contributions;
	}
	...
	public String toString() {
		...
		sb.append("\n\nTaskInfo:\n  tid\t#passes\t#contributions");
		for (int index = 0; index < this.passes.length; index++) {
			sb.append("\n  " + (index)); sb.append("\t");
			sb.append(this.passes[index]); sb.append("\t");
			sb.append(this.contributions[index]);
		}
